...
public class UserController {
    ...
    @GetMapping({"{id}/","{id}"})
    public User findOne(@PathVariable int id) {return this.userDao.findById(id).get();}

    @PostMapping({"/", ""})
    public void addUser(@RequestBody User user) {this.userDao.save(user);}

    @DeleteMapping({"/", ""})
    public void deleteUser(@PathVariable int id ) {this.userDao.deleteById(id);}

    @GetMapping({"connect/","connect"})
